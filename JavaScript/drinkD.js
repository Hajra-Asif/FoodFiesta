...
let display = async () => {
    ...
    if (idd) {
         let gettt = document.getElementById("con")
        // let fetchh = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${drinkId}`);
        let fetchh = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${idd}`);

        let jsonData = await fetchh.json();
        const drinks = jsonData.drinks[0];

        // If no drink found, show a message
        if (!drinks) {
            
            gettt.innerHTML = `<p>Drink not found.</p>`;
        }
    ...
    <div class="row mb-4">
        <div class="col-lg-6 mb-4">
            <img id="recipeImage" src="${drinks.strDrinkThumb}" alt="${drinks.strDrinkThumb}" class="img-fluidMeal rounded">
        </div>
        <div class="col-lg-6">
            <div id="title">
                <h1 id="recipeTitle" class="mb-1">${drinks.strDrink}</h1>
